...
class patTemplate
{
     ...
     * @see      addVar(), addRows(), addGlobalVar(), addGlobalVars()
     */
    public function addObject($template, $object, $prefix = '')
    {
        if (is_array($object)) {
            $rows = array();
            foreach ($object as $o) {
    ...
    */
     ...
     * @return   boolean     true on success, patError otherwise
     */
    public function applyOutputFilter($filter, $params = array())
    {
        if (!is_object($filter)) {
     ...
     * @return   object
     */
    public function &loadModule($moduleType, $moduleName, $params = array())
    {
    ...
    public function parseTemplate($template, $mode = 'w')
    {
         ...
         * check for autoclear
         */
        if (isset($this->_templates[$template]['attributes']['autoclear']) &&
     ...
     * @param    string
     */
    public function _parseDependencies($template)
    {
     ...
     * @param    string
     */
    public function _handleUnusedVars($template)
    {
     ...
     */
    public function getParsedTemplate($name = null)
    {
        if (is_null($name)) {
            $name = $this->_root;
        }
